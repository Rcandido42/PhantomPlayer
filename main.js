...
function searchSteamGames(query) {
  return new Promise((resolve) => {
    https.get(`https://store.steampowered.com/api/storesearch/?term=${encodeURIComponent(query)}&l=english&cc=US`, (res) => {
      ...
      res.on('end', () => { try { resolve((JSON.parse(data).items || []).map(i => ({ appId: i.id, name: i.name, icon: i.tiny_image || '' }))); } catch { resolve([]); } });
    }).on('error', () => resolve([]));
  });
}
...
function setupIPC() {
  ...
  ipcMain.handle('steam:logout', async () => { if (steamClient.isFarming) stopFarming(); steamClient.logout(); updateTrayMenu(); return { success: true }; });
